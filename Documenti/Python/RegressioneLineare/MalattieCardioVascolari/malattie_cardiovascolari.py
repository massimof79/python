...
1. L’algoritmo realizza un sistema di predizione del rischio cardiovascolare basato su tecniche di Machine Learning supervisionato. 
L’obiettivo è classificare ciascun paziente come sano oppure a rischio di malattia cardiaca, 
utilizzando un modello di regressione logistica addestrato su dati clinici reali.
...
2. I dati vengono letti direttamente da una sorgente online e organizzati in un DataFrame. 
...
3. Nel dataset originale il grado di malattia è espresso con più valori interi; 
...
4. Prima dell’addestramento, i dati vengono standardizzati. 
Le variabili cliniche hanno infatti scale molto diverse tra loro: alcune sono espresse in anni, altre in milligrammi o in valori discreti. 
La standardizzazione riporta tutte le feature su una scala comparabile, centrata sulla media e con deviazione standard unitaria. 
Questo passaggio è particolarmente importante per la regressione logistica, che è sensibile alle differenze di scala tra le variabili.

5. L’addestramento del modello avviene tramite regressione logistica, un algoritmo di classificazione che stima la probabilità che un’osservazione appartenga alla classe “a rischio”. 
Il modello apprende, a partire dai dati di training, un insieme di pesi che quantificano l’influenza di ciascuna variabile clinica sulla probabilità finale di malattia.
...
""" x_standardizzato = (x − μ) / σ

dove μ è la media della variabile e σ è la deviazione standard. Il risultato è una feature con media zero e deviazione standard unitaria.
 """
